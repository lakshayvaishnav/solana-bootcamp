...
declare_id!("7qqKMfVAqhNZecAAVmWagnWZ7MzAWFiwtVaX3YZ5vBdB");
...
pub struct UpdateMovieReview<'info> {
    #[account(
        ...
        seeds = [title.as_bytes(), initializer.key().as_ref()],
        bump,
        realloc = DESCRIMINATOR + MovieAccountState::INIT_SPACE,
...
pub struct AddMovieReview<'info> {
    #[account(
        ...
        bump,
        payer = initializer,
        space = DESCRIMINATOR + MovieAccountState::INIT_SPACE,
    )]
    pub movie_review: Account<'info, MovieAccountState>,
    #[account(mut)]
    pub initializer: Signer<'info>,
